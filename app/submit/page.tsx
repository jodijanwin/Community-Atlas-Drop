...
const inputStyle = { borderColor: "#C8D8C8", background: "var(--bg-white)", color: "var(--text-dark)" };
const labelStyle = { color: "var(--text-dark)" };
const optionalStyle = { color: "var(--text-2)" };
...
export default function SubmitPage() {
  ...
  if (submitted) {
    return (
      <div style={{ background: "var(--bg-page)", minHeight: "calc(100vh - 56px)" }} className="flex items-center justify-center px-6">
        <div className="max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold mb-3" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "var(--text-dark)" }}>Thank you.</h2>
          <p className="text-sm leading-relaxed mb-6" style={{ color: "var(--text-3)" }}>Your submission will be reviewed and added to the next monthly atlas update. Every listing is verified before it goes in — that&apos;s what makes this worth trusting.</p>
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={() => setSubmitted(false)} className="px-5 py-2.5 rounded-lg text-sm font-semibold border" style={{ borderColor: "var(--border-light)", color: "var(--text-3)" }}>Submit Another</button>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ background: "var(--bg-page)", minHeight: "calc(100vh - 56px)" }}>
      <div className="max-w-2xl mx-auto px-6 py-12">
        <div className="mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold mb-3" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "var(--text-dark)" }}>Add to the Atlas</h1>
          <p className="text-sm leading-relaxed" style={{ color: "var(--text-3)" }}>You know something we don&apos;t. Maybe it&apos;s the neighbour on your concession who fixes small engines, the hall that&apos;s been quietly available for years, or the person giving away seedlings every spring. Add it here.</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <select required name="category" value={form.category} onChange={handleChange} className="w-full px-3 py-2.5 rounded-lg border text-sm outline-none" style={{ ...inputStyle, color: form.category ? "var(--text-dark)" : "var(--text-2)" }}>
          ...
          {/* Event date + time */}
          <div className="p-4 rounded-lg border-l-4" style={{ background: "rgba(107,148,51,0.05)", borderLeftColor: "#6B9433" }}>
          ...
          <div className="p-4 rounded-lg border-l-4" style={{ background: "rgba(27,117,188,0.04)", borderLeftColor: "#1B75BC" }}>
            <label className="block text-xs font-semibold mb-1.5" style={labelStyle}>What&apos;s missing in Scugog? <span className="font-normal" style={optionalStyle}>(optional)</span></label>
            <textarea name="missing" value={form.missing} onChange={handleChange} rows={3} placeholder="What would change your part of the township if it existed? Housing, transit, shared space, co-ops, anything." className="w-full px-3 py-2.5 rounded-lg border text-sm outline-none resize-none" style={inputStyle} />
          </div>
          ...
          <div className="pt-2">
            ...
            <p className="text-xs mt-3" style={{ color: "var(--text-2)" }}>All submissions are reviewed before publishing. We verify each listing by visit, call, or message.</p>
          </div>
        </form>
      </div>
    </div>
