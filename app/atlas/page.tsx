...
function AtlasContent() {
  ...
  return (
    <div style={{ background: "var(--bg-page)", height: "calc(100vh - 56px)", display: "flex", flexDirection: "column" }}>
      <div className="px-4 py-3 border-b shrink-0" style={{ background: "var(--bg-white)", borderColor: "var(--border-light)" }}>
        <div className="max-w-7xl mx-auto flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <input type="text" placeholder="Search resources, skills, places…" value={query} onChange={(e) => setQuery(e.target.value)} className="flex-1 max-w-sm text-sm px-3 py-1.5 rounded-lg border outline-none" style={{ borderColor: "var(--border-light)", background: "#F8FAF8", color: "#1A2433" }} />
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-full sm:w-80 lg:w-96 shrink-0 overflow-y-auto border-r" style={{ borderColor: "var(--border-light)", background: "var(--bg-page)" }}>
          {filtered.length === 0 ? (
...
export default function AtlasPage() {
  return (
    <Suspense fallback={<div style={{ background: "var(--bg-page)", height: "calc(100vh - 56px)" }} />}>
      <AtlasContent />
    </Suspense>
  );
}
