...
export default function HomePage() {
  return (
    <div>
      ...
      {/* ── HERO — dark ─────────────────────────────────────── */}
      <section className="relative overflow-hidden px-6 pt-20 pb-24 sm:pt-32 sm:pb-36" style={{ background: "#1A2433" }}>
      ...
      {/* ── ABUNDANCE BY THE NUMBERS — light ────────────────── */}
      <div className="border-b px-6 py-8" style={{ background: "white", borderColor: "#D8E4D8" }}>
        <div className="max-w-4xl mx-auto">
          <p className="text-center text-xs font-bold uppercase tracking-widest mb-6" style={{ color: "#8AA0A8" }}>The economy of care — quantified</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-10 sm:gap-20">
            {STATS.map(({ value, label }) => (
              <div key={label} className="text-center">
                <p className="text-4xl font-bold" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1B75BC" }}>{value}</p>
                <p className="text-xs mt-1 uppercase tracking-wider" style={{ color: "#5A7080" }}>{label}</p>
              </div>
            ))}
          </div>
          <p className="text-center text-xs mt-6 italic" style={{ color: "#8AA0A8" }}>
            Most of the work that holds this community together goes uncounted. This atlas counts some of it.
          </p>
        </div>
      </div>

      {/* ── WHAT'S IN THE ATLAS — light ──────────────────────── */}
      <section className="px-6 py-16" style={{ background: "#F0F4F0" }}>
        <div className="max-w-4xl mx-auto">
          <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "#1B75BC" }}>Mapping our common treasures</p>
          <h2 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1A2433" }}>What's in the Atlas</h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {CATEGORIES.map((cat) => {
              ...
              return (
                <Link key={cat} href={`/atlas?category=${encodeURIComponent(cat)}`} className="p-4 rounded-lg border transition-all hover:scale-[1.02] hover:shadow-sm" style={{ background: "white", borderColor: "#D8E4D8" }}>
                  <div className="w-3 h-3 rounded-full mb-3" style={{ background: CATEGORY_COLORS[cat] }} />
                  <p className="text-sm font-semibold leading-tight" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1A2433" }}>{cat}</p>
      ...
      {/* ── STORIES OF SOLIDARITY — white ───────────────────── */}
      <section className="px-6 py-16 border-t" style={{ background: "white", borderColor: "#D8E4D8" }}>
        <div className="max-w-4xl mx-auto">
          <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: "#6B9433" }}>Caring in action</p>
          <h2 className="text-3xl font-bold mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1A2433" }}>Stories of Solidarity</h2>
          <p className="text-sm mb-8 max-w-xl" style={{ color: "#5A7080" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
            {stories.map((story) => (
              <div key={story.id} className="p-5 rounded-lg border flex flex-col" style={{ background: "#F8FAF8", borderColor: "#D8E4D8" }}>
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs font-semibold px-2 py-0.5 rounded" style={{ background: "rgba(107,148,51,0.12)", color: "#6B9433" }}>{story.source}</span>
                  <span className="text-xs" style={{ color: "#8AA0A8" }}>{story.date}</span>
                </div>
                <h3 className="text-sm font-semibold leading-snug mb-3 flex-1" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1A2433" }}>{story.title}</h3>
                ...
                {story.url ? (
                  ...
                  <span className="text-xs italic" style={{ color: "#8AA0A8" }}>Community-submitted</span>
                )}
              </div>
            ))}
          </div>
          <p className="text-xs mt-6 text-center" style={{ color: "#8AA0A8" }}>
      ...
      <section className="px-6 py-14 border-t" style={{ background: "#F0F4F0", borderColor: "#D8E4D8" }}>
        <div className="max-w-3xl mx-auto text-center">
          <p className="text-2xl sm:text-3xl font-semibold leading-snug mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1A2433" }}>
            &ldquo;Conventional advocacy says, <em>look what we lack.</em><br />
            <span style={{ color: "#1B75BC" }}>This atlas says, look what we already have.</span>&rdquo;
          </p>
      ...
      <section className="px-6 py-14 border-t" style={{ background: "#F0F4F0", borderColor: "#D8E4D8" }}>
        <div className="max-w-4xl mx-auto">
          <div className="flex items-end justify-between mb-6">
            ...
            <Link href="/atlas" className="text-sm" style={{ color: "#1B75BC" }}>See all {listings.length} →</Link>
          </div>
      ...
      {/* ── PRINT CTA BANNER — blue tint ────────────────────── */}
      <section className="px-6 py-10 border-t" style={{ background: "rgba(27,117,188,0.06)", borderColor: "#D8E4D8" }}>
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <p className="text-base font-semibold" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "#1A2433" }}>Print your own Community Atlas zine</p>
      ...
      {/* ── FOOTER — dark ───────────────────────────────────── */}
      <footer className="px-6 py-10 text-center" style={{ background: "#1A2433" }}>
