      ...
      <div className="max-w-3xl mx-auto px-6 py-12 sm:py-16">
        ...
        {upcoming.length > 0 ? (
                  ...
                  <div className="flex items-start gap-4">
                    <div className="shrink-0 text-center pt-0.5">
                      <div className="w-12 h-12 rounded-lg flex flex-col items-center justify-center" style={{ background: "#F6F1E8" }}>
                        <p className="text-xs font-bold uppercase leading-none" style={{ color: "#6B6158" }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs mb-2" style={{ color: "#7D8082" }}>
                        {formatDate(event.date)} · {event.time}
                      </p>
                      <p className="text-xs mb-2" style={{ color: "#6B6158" }}>📍 {event.location}</p>
                      <p className="text-sm leading-relaxed" style={{ color: "#6B6158" }}>{event.description}</p>
                      <div className="mt-3 flex flex-wrap items-center gap-4">
                        {event.recurring && (
                          <span className="text-xs italic" style={{ color: "#7D8082" }}>↻ {event.recurring}</span>
                        )}
                        {event.contact && (
          ...
          <div className="mb-14 p-8 rounded-lg border text-center" style={{ background: "white", borderColor: "#C2D1DB" }}>
            <p className="text-sm" style={{ color: "#7D8082" }}>No upcoming events listed yet.</p>
        ...
        {past.length > 0 && (
          <section className="mb-14">
            <h2 className="text-sm font-bold uppercase tracking-widest mb-6" style={{ color: "#7D8082" }}>Past Events</h2>
              ...
              {past.map((event) => (
                <div key={event.id} className="p-4 rounded-lg border flex items-center gap-4" style={{ background: "white", borderColor: "#C2D1DB", opacity: 0.7 }}>
                  <div className="shrink-0 w-10 text-center">
                    <p className="text-xs font-bold uppercase" style={{ color: "#7D8082" }}>
                      {new Date(event.date + "T12:00:00").toLocaleDateString("en-CA", { month: "short" })}
                    </p>
                    <p className="text-lg font-bold leading-none" style={{ fontFamily: "'Lora', serif", color: "#7D8082" }}>
                      {new Date(event.date + "T12:00:00").getDate()}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-semibold" style={{ fontFamily: "'Lora', serif", color: "#6B6158" }}>{event.title}</p>
                    <p className="text-xs" style={{ color: "#7D8082" }}>{event.location}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="p-6 rounded-lg border-l-4" style={{ background: "white", borderLeftColor: "#7A9E7E" }}>
          ...
          <p className="text-sm mb-4" style={{ color: "#6B6158" }}>A repair night, a market, a community meeting, a skill share — if it&apos;s in North Durham and open to neighbours, add it.</p>
